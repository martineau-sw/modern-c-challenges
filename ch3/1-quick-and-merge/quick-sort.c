...
void quick_sort_doubles(size_t low, size_t high, double A[]) {
    ...
    swap_doubles((high+low)/2, high, high - ((high+low)/2), A);
    size_t L = low-1;
    size_t R = high;
    while (1) {
        ...
        if (L >= R) break; 
        swap_doubles(L, R, R - L + 1, A);
    }
    swap_doubles(L, high, high - L + 1, A);
...
void quick_sort_strings(size_t low, size_t high, size_t buflen, char A[][buflen]) {
    if (((signed)high-low) <= 0) return;
    swap_strings((high+low)/2, high, high - ((high+low)/2) + 1, buflen, A);
    size_t L = low-1;
    size_t R = high;
    for(;;) {
        ...
        if (L >= R) break; 
        swap_strings(L, R, R - L + 1, buflen, A);
    }
    swap_strings(L, high, high - L + 1, buflen, A);
